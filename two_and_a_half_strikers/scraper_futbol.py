...
for name, (pid, slug) in players.items():
    ...
    try:
        ...
        nuevos = goals - old_goals
        if nuevos > 0:
            nuevos_goles.append((name, nuevos))
...
# =========================================================
# 🔧 MODO PRUEBA: simula un gol nuevo de Luis Suárez (quítalo o comenta esta línea cuando no pruebes)
#nuevos_goles.append(("Luis Suárez", 1))
# =========================================================

# Enviar notificaciones personalizadas si hay nuevos goles
if nuevos_goles:
    for name, nuevos in nuevos_goles:
        plural = "es" if nuevos > 1 else ""
        if "Mbappé" in name:
            msg = f"🐢 ¡La Tortugaaaaaaa sigue sumando! ⚽ {name} marcó {nuevos} gol{plural} más 🐢🔥"
        elif "Suárez" in name:
            msg = f"🦁 ¡O mundooooo sabe queeee... Suárez no cree en nadie! 💚 {nuevos} gol{plural} más para la cuenta 🦁"
        elif "Vlahović" in name:
            msg = f"🦓 ¡Mi amoreeeee! Vlahoviiiiiiiic lo volvió a hacer 💥 {nuevos} gol{plural} más 🦓"
        else:
            msg = f"⚽ ¡Gol de {name}! {nuevos} gol{plural} esta jornada 🔥"
